...
interface TareasApi {
    ...
    suspend fun guardar(@Query("usuarioId") usuarioId: Long, @Body tarea: Tarea): Tarea

    @PUT("api/tareas/actualizar/{id}")
    suspend fun actualizar(@Path("id") id: Long, @Query("usuarioId") usuarioId: Long, @Body tarea: Tarea): Tarea
